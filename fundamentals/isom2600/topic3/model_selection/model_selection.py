...
#%load_ext autoreload
#%autoreload 1
#%aimport model_selection.model_selection

def best_subset(X,Y):
    df = best_subset_init(X, Y)
    df_s1=best_subset_stage1(df)
    visual_best_allinone(df_s1)
    #pd.set_option('display.max_colwidth', -1)
    display(df_s1)
    return best_subset_stage2_all(df_s1)
...
def visual_stage2_min(df_s1,criteria):
    ylabel_list={'SSE': 'SSE', 'R_squared': 'R-squared', 'Adj_R2': 'Adj.R-square',
     'AIC': 'AIC', 'BIC': 'BIC', 'C_p':'Cp', 'MSE_validation':'Validation set error (MSE)','MSE_CV':'CV error (MSE)'} 
    plt.plot(df_s1.numb_features,df_s1[criteria],'o--',color = 'darkblue', label = 'Best subset')
    plt.plot(df_s1.loc[df_s1[criteria].idxmin(),'numb_features'],df_s1[criteria].min(), marker = 'x', markersize = 20,c='r')
    plt.xlabel('# Features')
...
def visual_stage2_max(df_s1,criteria):
    ylabel_list={'SSE': 'SSE', 'R_squared': 'R-squared', 'Adj_R2': 'Adj.R-square',
     'AIC': 'AIC', 'BIC': 'BIC', 'C_p':'Cp', 'MSE_validation':'Validation set error (MSE)','MSE_CV':'CV error (MSE)'} 
    plt.plot(df_s1.numb_features,df_s1[criteria],'o--',color = 'darkblue', label = 'Best subset')
    plt.plot(df_s1.loc[df_s1[criteria].idxmax(),'numb_features'],df_s1[criteria].max(), marker = 'x', markersize = 20,c='r')
    plt.xlabel('# Features')
    plt.ylabel(ylabel_list.get(criteria))
    plt.title(criteria)
    #plt.title(criteria+' - Best subset selection')
...
def best_subset_stage2_validation(df_s1,train_set,y,prop,rand_state):
    ...
    df_validation = pd.DataFrame({'numb_features':range(1,N_predictor+1), 'MSE_validation': mse_validate_list,  'features':M_list})
...
def best_subset_stage2_CV(df_s1,train_set,y,K):
    ...
    for i in range(0, N_predictor): 
        ...
        y_CV = train_set[y]
        scores = cross_val_score(lm, X_CV, y_CV, cv=K, scoring='neg_mean_squared_error')#inpunt 'cv' is used to control K
...
def visual_stage2_valdiation(df_validation):
    ...
    plt.plot(df_validation.loc[df_validation['MSE_validation'].idxmin(),'numb_features'],df_validation['MSE_validation'].min(), 
...
def forward_selection_stage1(X, Y):
    ...
    features = []
    sequence = []
    SSE_list, R_squared_list, adj_R2_list, AIC_list, BIC_list, Cp_list= [np.inf], [np.inf], [np.inf], [np.inf], [np.inf], [np.inf]  # Due to 1 indexing of the loop...
    features_list = dict()
